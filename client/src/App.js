...
function App() {
  ...
  const renderRatingStars = (num) => {
    let ratingArr = [];
    const starSvg = (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="#F3B73E">
  ...
  return (
    ...
    >
        ...
        <React.Fragment key={entry._id}>
          ...
          {showPopup[entry._id] ? (
            ...
            >
              <div className="popup">
                <h3>{entry.title}</h3>
                <div className="rating">{renderRatingStars(entry.rating)}</div>
                <p>{entry.comments}</p>
      ...
      {addEntryLocation ? (
        <>
          <Marker longitude={addEntryLocation.longitude} latitude={addEntryLocation.latitude}>
            <svg
